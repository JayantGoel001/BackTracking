...
bool ratInAMazeUtil(int **ar,int n,int x,int y,int **path)
{
    ...
    else
    {
        ...
        {
            if(isSafe(ar,n,(x+i),y)==true)
            {
                path[x+i][y]=1;
                if(ratInAMazeUtil(ar,n,x+i,y,path)==true)
                {
                    return true;
                }
                else
            ...
            if(isSafe(ar,n,x,y+i)==true)
            {
                path[x][y+i]=1;
                if(ratInAMazeUtil(ar,n,x,y+i,path)==true)
                {
                    return true;
                }
                else
...
void ratInAMaze(int **ar,int n)
{
    ...
    path[0][0]=1;
    path[n-1][n-1]=1;
    if(ratInAMazeUtil(ar,n,0,0,path)==true)
    {
